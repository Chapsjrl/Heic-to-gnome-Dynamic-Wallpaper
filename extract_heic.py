...
def process_image(i):
    with Image(filename=f"{in_heic}[{i}]") as img:
        ...
        if img.width / img.height != 16 / 9:
            ...
            img.crop(0, (img.height - new_height) // 2,
                     width=img.width, height=new_height)
...
if __name__ == "__main__":
    ...
    with open(in_heic, "rb") as fileobj:
        for line in fileobj:
            ...
            if start_index != -1:
                match = re.match(r"apple_desktop:(\w+)=\"(.+?)\"",
                                 line[start_index:].decode("latin1"))
                metadata_type = match.group(1)
                metadata_b64 = match.group(2)

    metadata_plist = plistlib.loads(base64.b64decode(
        metadata_b64), fmt=plistlib.FMT_BINARY)
    ...
    print(
        f"Saved metadata of type apple_desktop:{metadata_type} to metadata.xml")
    ...
    subprocess.run(f"python3 linux_convert.py {out_dir}", shell=True)
    print("done!")
